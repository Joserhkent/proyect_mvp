...
interface Props {
  clientes: Cliente[];
  cotizacionesPorCliente: Record<string, number>;
}

export function ClientesClient({ clientes, cotizacionesPorCliente }: Props) {
          ...
          {filtered.map((cli) => (
            <div key={cli.id} className="p-5 rounded-xl bg-white border border-slate-200 shadow-xs space-y-3 flex flex-col justify-between">
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex justify-between items-center text-xs">
                ...
                <span className="font-bold text-emerald-700">{cotizacionesPorCliente[cli.id] ?? 0}</span>
              </div>
            </div>
          ))}
        </div>
